...
sf::Packet& operator >>(sf::Packet& packet, ActionOutcome& outcome){
    ...
    packet >> int32; outcome.action = int32;

    packet >> int32; outcome.idActor = int32;
    packet >> int32; outcome.idEnemy = int32;

    packet >> int32; outcome.idWinner = int32;
    packet >> int32; outcome.idLoser = int32;


    packet >> iValue;
    for (int i = 0; i < iValue; i++){
        UnitActionOutcome uao;
        packet >> int32; uao.id = (UnitActionOutcome::Outcome)int32;
        packet >> int32; uao.idCauser = int32;
        packet >> int32; uao.idTarget = int32;
    ...
    packet >> int32; outcome.nTargets = int32;
    packet >> int32; outcome.nActors = int32;
    ...
    packet >> int32; outcome.hero = int32;
    packet >> int32; outcome.unit = int32;

    //comp
    packet >> iValue; sf::Int32(outcome.armyComposition.size());
    for (int i = 0; i < iValue; i++){
        pair<CUnit::ID, int> p;
        packet >> int32; p.first = int32;
        packet >> int32; p.second = int32;
        outcome.armyComposition.insert(p);
    }

    packet >> int32; outcome.formation = (CArmy::Formation)int32;
    packet >> int32; outcome.dmgType = (CUnit::DamageType)int32;

    packet >> int32; outcome.iValue = int32;
    packet >> outcome.dValue;
    packet >> sValue; outcome.sValue = sValue;
    packet >> outcome.bValue;
