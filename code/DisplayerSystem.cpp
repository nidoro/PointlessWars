...
void DisplayerSystem::update() {
    for(EntityListIt i = entities.begin(); i != entities.end(); i++) {
        ...
        case CDisplayer::SPIN_BUTTON:
            text = int2str(eValue->get<CSpinButton>()->value);
            break;
        case CDisplayer::SECTOR_STRENGTH:
            text = int2str(eValue->get<CSector>()->strength);
            break;
        case CDisplayer::UNIT_COUNT:
            text = int2str(getUnitCount(eValue));
            break;
        case CDisplayer::RESISTANCE_NEUTRAL:
            text = int2str(eValue->get<CUnit>()->resistance[CUnit::EARTH]);
            break;
        case CDisplayer::RESISTANCE_FIRE:
            text = int2str(eValue->get<CUnit>()->resistance[CUnit::FIRE]);
            break;
        case CDisplayer::RESISTANCE_WATER:
            text = int2str(eValue->get<CUnit>()->resistance[CUnit::WATER]);
            break;
        case CDisplayer::RESISTANCE_ELECTRIC:
            text = int2str(eValue->get<CUnit>()->resistance[CUnit::AIR]);
            break;
        case CDisplayer::INPUT_TEXT:
            text = eValue->get<CInputTextBox>()->content;
            break;
        case CDisplayer::BATTLE_STATE_DESCRIPTION:
            text = eValue->get<CBattle>()->stateDescription;
            break;
        case CDisplayer::DROP_LIST_VALUE:
            text = eValue->get<CDropList>()->value;
            break;
        case CDisplayer::ALIVE_COUNT:
            text = int2str(eValue->get<CArmy>()->nAlive);
            break;
        case CDisplayer::STR_TOGGLE_BUTTON:
            text = eValue->get<CStringToggleButton>()->values[eValue->get<CStringToggleButton>()->valueIndex];
            break;
        case CDisplayer::GAME_STATE:
            text = getGameStateString();
            break;
        default:
            break;
        }
